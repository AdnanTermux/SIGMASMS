...
$validGroups = ['DATE(sr.received_at)' => 'Date', 'sr.service' => 'Service', 'sr.country' => 'Country', 'sr.number' => 'Number'];
$allowedGroupKeys = array_keys($validGroups);
...
foreach ($groupBy as $g) {
    if (in_array($g, $allowedGroupKeys)) $groupByFields[] = $g;
}
...
if (!empty($groupByFields)) {
    ...
    $dataSql    = "$selectSql $joinSql GROUP BY $groupByStr";
    $orderSql   = " ORDER BY last_received DESC LIMIT $length OFFSET $start";
} else {
    ...
    $dataSql   = "$selectSql $joinSql";
    $orderSql  = " ORDER BY sr.received_at DESC LIMIT $length OFFSET $start";
}

// Total count (unfiltered)
$totalStmt = $pdo->query("SELECT COUNT(*) FROM sms_received");
...
// Data
$dataStmt = $pdo->prepare($dataSql . $orderSql);
...
foreach ($rows as $r) {
    if (!empty($groupByFields)) {
        ...
        $row = [
            ...
            h($r['country'] ?? '–'),
            '<strong>' . h($r['otp'] ?? '') . '</strong>',
            '<small class="text-muted">' . h(mb_substr($r['message'] ?? '', 0, 80)) . '</small>',
            '$' . number_format((float)$r['rate'], 6),
            '<span class="text-success">$' . number_format((float)$r['profit_amount'], 6) . '</span>',
