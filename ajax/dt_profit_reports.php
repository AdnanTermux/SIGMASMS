...
if (!in_array($role, ['admin','manager'])) {
    ...
    if (empty($userIds)) {
        echo json_encode(['draw'=>$draw,'recordsTotal'=>0,'recordsFiltered'=>0,'data'=>[],'footer'=>['total_profit'=>'0','total_sms'=>0]]);
        exit;
    }
...
$totalCount = $pdo->query("SELECT COUNT(DISTINCT $groupExpr) FROM profit_log pl JOIN sms_received sr ON sr.id = pl.sms_received_id JOIN numbers n ON n.id = pl.number_id")->fetchColumn();
